...
const PhysicalItemProcess = ({ contract, auctionId, isSeller, isBuyer }) => {
    ...
    useEffect(() => {
        const fetchEscrowAmount = async () => {
            try {
                const amount = await contract.getEscrowAmount(auctionId);
                setEscrowAmount(ethers.utils.formatEther(amount));
            } catch (err) {
                console.error('Error fetching escrow amount:', err);
            }
        };
    ...
    const handleFundEscrow = async () => {
        try {
            setLoading(true);
            const tx = await contract.fundEscrow(auctionId, {
                        ...
                        <div className="flex items-start">
                            ...
                            <div>
                                ...
                                <p className="text-yellow-400 text-sm">
                                    This is a physical item auction. You need to fund the escrow ({escrowAmount} ETH) before proceeding.
                                </p>
                                <button
...
export default PhysicalItemProcess; 