...
const UserProfile = () => {
    ...
    const userContractAddress = process.env.REACT_APP_USER_CONTRACT_ADDRESS || '0x0000000000000000000000000000000000000000';
    const auctionFactoryAddress = process.env.REACT_APP_AUCTION_FACTORY_ADDRESS || '0x0000000000000000000000000000000000000000';
    const rpcUrl = process.env.REACT_APP_RPC_URL || 'http://localhost:8545';

    // Helper function to format ether values (compatible with ethers.js v5)
    const formatEther = (value) => {
        try {
            // Use ethers.js v5 format
    ...
    // Contract read hooks with better error handling
    const { data: userProfileData, isError: isUserProfileError, isLoading: isUserProfileLoading } = useContractRead({
        address: userContractAddress,
        abi: UserABI,
        functionName: 'getUserProfile',
        args: [address],
        enabled: !!address && !!userContractAddress && userContractAddress !== '0x0000000000000000000000000000000000000000'
    });

    const { data: createdAuctions, isError: isCreatedAuctionsError } = useContractRead({
        address: userContractAddress,
        abi: UserABI,
    ...
    const { data: userPurchases, isError: isUserPurchasesError } = useContractRead({
        address: userContractAddress,
        abi: UserABI,
    ...
    // Contract write hooks with better error handling
    const { writeAsync: createUser, isError: isCreateUserError } = useContractWrite({
        address: userContractAddress,
        abi: UserABI,
    ...
    const { writeAsync: updateProfile, isError: isUpdateProfileError } = useContractWrite({
        address: userContractAddress,
        abi: UserABI,
    ...
    const { writeAsync: becomeSeller, isError: isBecomeSellerError } = useContractWrite({
        address: userContractAddress,
        abi: UserABI,
    ...
    // Event listening for BidPlaced events with better error handling
    useContractEvent({
        ...
        enabled: !!address && !!auctionFactoryAddress && auctionFactoryAddress !== '0x0000000000000000000000000000000000000000'
    });

    // Event listening for AuctionWon events with better error handling
    useContractEvent({
        address: auctionFactoryAddress,
        abi: AuctionFactoryABI,
    ...
    // Event listening for AuctionCreated events with better error handling
    useContractEvent({
        address: auctionFactoryAddress,
        abi: AuctionFactoryABI,
    ...
    // Load historical data on component mount with better error handling
    useEffect(() => {
        if (address && auctionFactoryAddress && auctionFactoryAddress !== '0x0000000000000000000000000000000000000000' && rpcUrl) {
            try {
    ...
    useEffect(() => {
        if (userProfileData) {
            ...
            console.error('Error loading user profile:', isUserProfileError);
            setError('Failed to load user profile. Please make sure the UserContract is deployed and the address is correct.');
            setLoading(false);
        } else if (userContractAddress === '0x0000000000000000000000000000000000000000') {
            setError('UserContract address is not set. Please update the .env file with the correct address.');
            setLoading(false);
        }
    }, [userProfileData, isUserProfileError, userContractAddress]);
    ...
    if (error) {
        return (
            <div className="bg-gray-800 rounded-lg p-6">
                ...
                <p className="text-white">There was an error loading your profile. Please try again later.</p>
            </div>
        );
    }
